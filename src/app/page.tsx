...
export default function LandingPage() {
  ...
  const AuthModal = () => {
    ...
    const switchLabel = isSignIn ? "Don't have an account?" : 'Already have an account?';
    const switchAction = () => { setAuthModal(isSignIn ? 'signup' : 'signin'); setAuthError(''); };
    const submitFn = isSignIn ? handleSignIn : handleSignUp;
        ...
        <div className="w-full max-w-md bg-background border border-border rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-6 space-y-4">
            ...
            <p className="text-[10px] text-center text-muted-foreground/70">
              {switchLabel}{' '}
              <button onClick={switchAction} className="text-blue-400 hover:text-blue-300 font-semibold cursor-pointer">{isSignIn ? 'Sign up' : 'Sign in'}</button>
            </p>
          </div>
        </div>
      </div>
